...
local SOCKET_SERVER_URL = "scspdrone.mithran.org" -- Example: "http://localhost:3000"
-----------------------------------------------
...
function Reader:Connect(targetWebSocketUrl: string): RoSocketConnectionResponseData
	...
	-- Validate the targetWebSocketUrl (e.g., your Gazebo orchestrator's ws:// address)
	-- The RoSocket library description implies it handles ws(s) URLs passed to its backend.
	-- No need to validate it here if the RoSocket backend does it.

	local requestBody = {Socket = tostring(targetWebSocketUrl)}
	local requestParameters = {
		Url = string.gsub(targetWebSocketUrl, "ws", "http", 1) .. Dictionary.Connection,
		Method = "POST",
	...
	if not Response or not Response.Success then
		warn(self:FormatText(`Failed to reach RoSocket backend at {SOCKET_SERVER_URL} | StatusCode: {Response and Response.StatusCode or "N/A"}, Body: {Response and Response.Body or "N/A"}`))
